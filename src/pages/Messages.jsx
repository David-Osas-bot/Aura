...
export default function Messages() {
  ...
  // Clear unread count when user opens this thread
  useEffect(() => {
    if (!uid) return;
    auth.currentUser?.getIdToken().then((token) => {
      fetch(`${import.meta.env.VITE_API_URL}/api/notifications/clear-unread`, {
        method: 'POST',
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      fetch(`${import.meta.env.VITE_API_URL}/api/notifications/message-received`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          messagePreview: text.trim().slice(0, 80),
        }),
